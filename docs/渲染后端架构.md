...
- dirty rect 重绘写入 render cache；resolve 到 swapchain 必须全量执行，不依赖 present 后 swapchain image 保留旧内容。
...
- Vulkan render cache 第二段已接入 offscreen image/framebuffer 和 cache-to-swapchain 全量 resolve；支持 transfer copy，也支持无 transfer dst 时用采样绘制 resolve。
- Vulkan 静态图片 GPU texture 已按 `image_source` cache key 复用；GIF 仍使用独占动态 texture，避免共享 texture 被帧更新污染。
